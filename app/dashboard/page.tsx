...
const COUNTRIES: { key: Country; flag: string; name: string }[] = [
...
function UserDashboardInner() {
  ...
  useEffect(() => {
    ...
    // Allow public access to ranking, pizarra and simulator
  ...
  const loadData = useCallback(async () => {
    ...
    if (mDays) {
      ...
      const alreadyHasPreds = preds?.some((p:any) => firstDayMatchIds.includes(p.match_id))
  ...
  const countryGlow: React.CSSProperties | undefined = league === 'kings' ? {
    boxShadow:
      country === 'spain'
  ...
  const leftNavItems = [
    {
      label: 'KINGS',
      active: view === 'picks' && league === 'kings',
      activeColor: '#ffd300',
      onClick: () => { setView('picks'); setLeague('kings') },
    },
    {
      label: 'QUEENS',
      active: view === 'picks' && league === 'queens',
      activeColor: '#01d6c3',
      onClick: () => { setView('picks'); setLeague('queens') },
    },
    {
      label: 'RANKING',
      active: view === 'ranking',
  ...
  const rightNavItems = [
    {
      label: 'PICKS',
      active: view === 'all-picks',
      activeColor: '#ffffff',
      onClick: () => setView('all-picks'),
    },
    {
      label: 'SIMULADOR',
  ...
  // Country selector only in picks view, and only for Kings (Queens = Spain only, no tabs)
  const showCountrySelector = view === 'picks' && league === 'kings'
  const availableCountries = COUNTRIES.map(c => ({ ...c, disabled: false }))
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans overflow-x-hidden">
      ...
      <main className={`mx-auto pt-6 pb-4 ${view === 'all-picks' || view === 'pizarra' ? 'w-full px-4' : 'max-w-5xl px-4'}`}>
        {/* Country selector — only in picks view */}
        {showCountrySelector && (
          <div className="flex gap-2 mb-5 max-w-2xl mx-auto overflow-x-auto pb-1">
            {availableCountries.map(({ key, flag, name, disabled }) => (
              <button
                key={key}
                disabled={disabled}
              ...
              >
                <span>{flag}</span>
                <span>{name}</span>
                {disabled && <span className="text-[8px] not-italic ml-1 text-slate-600">PRÓXIMAMENTE</span>}
              </button>
            ))}
          </div>
        )}
    ...
    <div className="w-full flex flex-col items-center">
      ...
      {activeMatchday && (
        <div className="w-full">
          <div className="w-full px-4 sm:px-6 py-8 flex flex-row justify-center items-center gap-4 md:gap-8 bg-gradient-to-b from-black/20 to-transparent relative">
            {totalPages > 1 && (
              <div className="flex items-center bg-black/60 rounded-xl border border-white/10 overflow-hidden shadow-2xl">
                <button 
                  disabled={currentPage === 0} 
                  onClick={() => setCurrentPage(prev => prev - 1)} 
                  className={`px-5 sm:px-6 py-2 text-sm font-black transition-colors border-r border-white/10 ${currentPage === 0 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}
                >◀</button>
                <button 
                  disabled={currentPage === totalPages - 1} 
                  onClick={() => setCurrentPage(prev => prev + 1)} 
                  className={`px-5 sm:px-6 py-2 text-sm font-black transition-colors ${currentPage === totalPages - 1 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}
                >▶</button>
              </div>
            )}
  ...
  return (
    <div className="w-full flex flex-col gap-4">
      ...
      <div 
        ...
        className="relative w-full aspect-video bg-slate-900 overflow-hidden border-2 border-slate-700 shadow-2xl touch-none"
        ...
        {playersOnPitch.map((player) => (
          <div 
            ...
            style={{
              ...
              width: 'clamp(36px, 9vw, 160px)',
              height: 'clamp(36px, 9vw, 160px)',
            }}
          >
...
function SimulatorView() {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="w-full flex justify-center items-center flex-wrap gap-4 py-3 px-4 sm:px-6 border-b border-white/5">
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          ...
          <div className="w-full xl:w-[480px]">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-[10px] uppercase font-bold text-slate-400 bg-black/20 p-3 rounded-xl border border-white/5">
              ...
              <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 bg-blue-500 rounded-full"></div><span>2º a 6º Cuartos</span></div>
              <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 bg-red-500 rounded-full"></div><span>7º a 10º Play In</span></div>
            </div>
          </div>
        </div>
      </div>
...
function RankingView({ user }: { user: any }) {
  ...
  useEffect(() => {
    const fetchRanking = async () => {
      ...
      // Use same logic as admin: only locked matchdays
                    ...
                    <tr key={u.username} className={`border-b border-white/5 hover:bg-white/[0.03] transition-colors group ${isFirst ? 'bg-[#FFD300]/5' : ''} ${isMe ? 'bg-blue-500/10' : ''}`}>
                      ...
                      <td className="w-[120px] px-3 py-1 border-r border-white/5">
                        <div className="flex items-center gap-3">
                          <div className={`relative w-7 h-7 rounded-full overflow-hidden border shrink-0 shadow-md flex items-center justify-center bg-slate-800 font-bold text-[10px] ${isFirst ? 'border-[#FFD300] text-[#FFD300]' : isMe ? 'border-white text-white' : 'border-white/10 text-slate-400'}`}>
