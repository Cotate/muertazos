...
function CompetitionReadOnly({ competitionKey }: { competitionKey: string }) {
    ...
    return (
        <div className="w-full flex flex-col items-center bg-slate-900/40 rounded-3xl border border-slate-800 shadow-2xl backdrop-blur-sm overflow-hidden mt-6 animate-in fade-in duration-700">
            ...
            {activeMatchday && (
                <div className="relative w-full mb-4">
                    ...
                    <div className="w-full px-4 md:px-10 py-4 flex justify-between items-center bg-slate-900/40 border-b border-white/5">
                        <div className="flex items-center gap-2">
                            {pageChunks.length > 1 && (
                                <div className="flex items-center bg-black/40 rounded border border-white/10 overflow-hidden">
                                    <button disabled={currentPage === 0} onClick={() => setCurrentPage(prev => prev - 1)} className="px-5 py-2 text-xs font-black hover:bg-white/10 text-[#FFD300] disabled:opacity-20 transition-colors">◀</button>
                                    <button disabled={currentPage === pageChunks.length - 1} onClick={() => setCurrentPage(prev => prev + 1)} className="px-5 py-2 text-xs font-black hover:bg-white/10 text-[#FFD300] disabled:opacity-20 transition-colors">▶</button>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="w-full overflow-x-auto">
                        <table className="w-full min-w-[1000px] border-collapse table-fixed text-center">
                            <thead>
                                <tr className="bg-black/60 text-[11px] text-slate-500 font-black uppercase tracking-tighter border-b border-white/5">
                                    <th className="w-[200px] p-2 border-r border-white/5 align-middle bg-slate-950/20">PARTIDO</th>
                                    {paginatedUsers.map(u => (
                                        <th key={u.id} className="py-2 px-1 border-r border-white/5 bg-black/20 text-slate-200 align-middle">
                                            <div className="flex flex-col items-center justify-center gap-1.5">
                                                <div className="relative w-10 h-10 rounded-full overflow-hidden border-2 border-white/10 bg-slate-800 shadow-lg">
                                                    <Image src={`/usuarios/${u.username}.jpg`} alt={u.username} fill sizes="40px" className="object-cover" />
                                                </div>
                                    ...
                                    <tr key={m.id} className="border-b border-white/5 hover:bg-white/[0.03]">
                                        <td className="py-2 px-2 border-r border-white/5 bg-slate-900/30">
                                            <div className="flex items-center justify-center gap-3">
                                                <div className={`${m.winner_team_id === m.home_team_id ? 'opacity-100 scale-110 drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : m.winner_team_id === null ? 'opacity-100' : 'opacity-15 grayscale scale-90'} transition-all`}>
                                                ...
                                                <div className={`${m.winner_team_id === m.away_team_id ? 'opacity-100 scale-110 drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : m.winner_team_id === null ? 'opacity-100' : 'opacity-15 grayscale scale-90'} transition-all`}>
                                                    ...
                                                    {pred?.predicted_team?.logo_file ? (
                                                        <div className="flex justify-center">
                                                            <Image 
                                                                ...
                                                                alt="p" 
                                                                className={`transition-all duration-300 ${m.winner_team_id ? (isHit ? 'opacity-100 scale-110 drop-shadow-[0_0_10px_rgba(255,211,0,0.5)]' : 'opacity-10 grayscale') : 'opacity-100'}`} 
                                                            />
                                                        </div>
