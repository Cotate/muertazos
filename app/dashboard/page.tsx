...
function CompetitionReadOnly({ competitionKey }: { competitionKey: string }) {
    ...
    const load = async () => {
        // Solo traemos las jornadas que están públicas (is_visible = true)
        const { data: mData } = await supabase.from('matchdays').select('*, matches(*, home:home_team_id(*), away:away_team_id(*))').eq('competition_key', competitionKey).eq('is_visible', true).order('display_order')
        ...
        if (mData) { 
            ...
            setActiveMatchdayId(prev => {
                if (!prev) return (mData.length > 0 ? mData[0].id : null);
                if (prev && !mData.find(d => d.id === prev)) return (mData.length > 0 ? mData[0].id : null);
                return prev;
            })
        }
                        ...
                        <div className="flex justify-start">
                            {totalPages > 1 && (
                                <div className="flex items-center bg-black/40 rounded border border-white/10 overflow-hidden">
                                    <button disabled={currentPage === 0} onClick={() => setCurrentPage(prev => prev - 1)} className={`px-3 md:px-5 py-2 text-xs font-black transition-colors border-r border-white/10 ${currentPage === 0 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>◀</button>
                                    <button disabled={currentPage === totalPages - 1} onClick={() => setCurrentPage(prev => prev + 1)} className={`px-3 md:px-5 py-2 text-xs font-black transition-colors ${currentPage === totalPages - 1 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>▶</button>
                                </div>
                            )}
                        </div>
