...
function SimulatorView() {
    ...
    // Escudos mucho más grandes para los marcadores
    const isPio = (filename: string) => filename?.toLowerCase().includes('pio');
    const getLogoSize = (filename: string) => isPio(filename) ? 80 : 110;
    ...
    return (
        <div className="w-full flex flex-col items-center">
            ...
            <div className="w-full max-w-7xl flex flex-wrap items-center justify-center gap-4 py-3 px-6 border-b border-white/10">
                ...
                <div className="flex flex-wrap justify-center gap-1.5">
                    {matchdays.map(day => {
                        ...
                        return (
                            <button key={day.id} onClick={() => setActiveMatchdayId(day.id)} className={`px-3 py-1.5 text-[10px] font-black italic rounded border transition-all ${isActive ? (compKey === 'kings' ? 'bg-[#FFD300] border-[#FFD300] text-black' : 'bg-[#01d6c3] border-[#01d6c3] text-black') : 'bg-transparent border-slate-700 text-slate-500 hover:border-slate-500'}`}>
                                {label}
                            </button>
                        )
                    })}
            ...
            <div className="w-full max-w-7xl mx-auto flex flex-col xl:flex-row gap-6 px-6 py-6">
                <div className="flex-1">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {activeMatchday?.matches?.map((m: any) => {
                            ...
                            return (
                                <div key={m.id} className="bg-slate-900/40 border border-white/5 rounded-2xl p-6 flex flex-col items-center justify-center shadow-lg">
                                    <div className="w-full flex items-center justify-between gap-4">
                                        <div className="flex flex-col items-center flex-1">
                                            {m.home && (
                                                <button onClick={() => isTie && togglePenaltyWinner(m.id, m.home_team_id)} className={`transition-all duration-300 ${isTie && s.penaltyWinnerId === m.home_team_id ? 'drop-shadow-[0_0_20px_#FFD300] scale-110' : isTie ? 'opacity-30 grayscale' : 'hover:scale-105'}`}>
                                        ...
                                        <div className="flex items-center gap-2">
                                            <input type="text" value={s.hg} onChange={(e) => handleLocalScoreChange(m.id, 'hg', e.target.value)} className="w-14 h-14 text-center bg-black border-2 border-white/10 rounded-xl font-black text-3xl text-white focus:border-[#FFD300] focus:outline-none transition-all" maxLength={2} />
                                            <span className="text-lg font-black text-white italic px-1">VS</span>
                                            <input type="text" value={s.ag} onChange={(e) => handleLocalScoreChange(m.id, 'ag', e.target.value)} className="w-14 h-14 text-center bg-black border-2 border-white/10 rounded-xl font-black text-3xl text-white focus:border-[#FFD300] focus:outline-none transition-all" maxLength={2} />
                                        </div>

                                        <div className="flex flex-col items-center flex-1">
                                            {m.away && (
                                                <button onClick={() => isTie && togglePenaltyWinner(m.id, m.away_team_id)} className={`transition-all duration-300 ${isTie && s.penaltyWinnerId === m.away_team_id ? 'drop-shadow-[0_0_20px_#FFD300] scale-110' : isTie ? 'opacity-30 grayscale' : 'hover:scale-105'}`}>
                ...
                {/* Clasificación con GF y GC restaurados */}
                            ...
                            <tbody>
                                {standings.map((t, idx) => (
                                    <tr key={t.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                                        <td className="relative py-2.5 font-black text-xs text-slate-400">
