...
export default function AdminDashboard() {
  ...
  return (
    /* ELIMINAMOS max-w-7xl y cualquier restricción. w-screen asegura el ancho total del monitor */
    <div className="min-h-screen bg-[#0a0a0a] pb-20 w-screen text-white overflow-x-hidden">
        
        {/* CABECERA - Centrada pero sin restringir el fondo */}
        <div className="relative flex items-center justify-center py-10 px-10">
        ...
        {/* NAVEGACIÓN - Alineada con el inicio de la tabla */}
        <div className="flex gap-6 mb-8 border-b border-slate-800 px-10 overflow-x-auto whitespace-nowrap scrollbar-hide">
        ...
        {/* CONTENEDOR DE CONTENIDO - Quitamos márgenes automáticos para que use todo el ancho disponible */}
...
function TabBtn({label, active, onClick, activeColor}: any) {
    return (
        <button 
            ...
            className="pb-4 px-2 font-black italic tracking-tighter transition-all uppercase text-base"
        >
            {label}
        </button>
    )
                ...
                <div key={day.id} className="bg-slate-900/40 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl w-full">
                    ...
                    <div className="p-6 bg-slate-800/40 flex justify-between items-center border-b border-slate-700/50">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 font-mono italic">{day.date_label || 'FECHA POR DEFINIR'}</p>
                        </div>
                        <div className="flex gap-3">
                            {/* BOTONES CON COLORES RESTAURADOS */}
                            <button 
                                onClick={()=>toggleVisible(day.id, day.is_visible)} 
                                className={`px-6 py-2 text-xs font-black rounded-xl border transition-all ${day.is_visible ? 'bg-green-600 border-green-400 text-white shadow-[0_0_15px_rgba(22,163,74,0.4)]' : 'bg-slate-800 border-slate-600 text-slate-400'}`}
                            >
                                {day.is_visible ? 'PÚBLICO' : 'OCULTO'}
                            </button>
                            <button 
                                onClick={()=>toggleLock(day.id, day.is_locked)} 
                                className={`px-6 py-2 text-xs font-black rounded-xl border transition-all ${day.is_locked ? 'bg-red-600 border-red-400 text-white shadow-[0_0_15px_rgba(220,38,38,0.4)]' : 'bg-blue-600 border-blue-400 text-white shadow-[0_0_15px_rgba(37,99,235,0.4)]'}`}
                            >
                    ...
                    {/* TABLA - Quitamos restricciones de ancho */}
                    <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-slate-700">
                        <table className="w-full border-collapse min-w-max">
                            <thead>
                                <tr className="text-xs text-slate-500 font-black uppercase tracking-widest">
                                    <th className="w-[220px] p-6 text-left sticky left-0 bg-[#0f172a] z-20 border-r border-slate-800">PARTIDO</th>
                                    {users.map(u => (
                                        <th key={u.id} className="p-2 text-center min-w-[140px]">
                                            <div className="bg-slate-800/60 rounded-lg py-3 px-2 text-slate-300 font-bold border border-slate-700/50">
                                                {u.username}
                                            </div>
                                        </th>
                                    ))}
                            ...
                            <tbody>
                                {day.matches?.map((m: any) => (
                                    <tr key={m.id} className="border-b border-slate-800/50 hover:bg-white/[0.02] transition-colors">
                                        <td className="p-4 sticky left-0 bg-[#0f172a] z-10 border-r border-slate-800 shadow-2xl">
                                            <div className="flex items-center justify-around gap-4 px-2">
                                                <button onClick={()=>setWinner(m.id, m.winner_team_id === m.home_team_id ? null : m.home_team_id)} className={`relative w-12 h-12 flex items-center justify-center rounded-2xl border-2 transition-all hover:scale-110 ${m.winner_team_id === m.home_team_id ? 'border-green-500 bg-green-500/20' : 'border-transparent opacity-40 hover:opacity-100'}`}>
                                                    {m.home && <Image src={`/logos/${folder}/${m.home.logo_file}`} width={40} height={40} alt="h" className="object-contain" />}
                                                </button>
                                                <span className="text-[10px] font-black text-slate-700 italic">VS</span>
                                                <button onClick={()=>setWinner(m.id, m.winner_team_id === m.away_team_id ? null : m.away_team_id)} className={`relative w-12 h-12 flex items-center justify-center rounded-2xl border-2 transition-all hover:scale-110 ${m.winner_team_id === m.away_team_id ? 'border-green-500 bg-green-500/20' : 'border-transparent opacity-40 hover:opacity-100'}`}>
                                                    {m.away && <Image src={`/logos/${folder}/${m.away.logo_file}`} width={40} height={40} alt="a" className="object-contain" />}
                                                </button>
                                            </div>
                                        </td>
                                        {users.map(u => {
                                                    ...
                                                    {pred?.predicted_team?.logo_file ? (
                                                        <div className="flex justify-center">
                                                            <Image 
                                                                ...
                                                                width={45} height={45} 
                                                                className={`object-contain transition-all duration-500 ${isHit ? 'drop-shadow-[0_0_12px_rgba(34,197,94,0.8)] scale-110' : 'opacity-20 grayscale'}`} 
                                                                alt="p" 
                                                            />
                                                        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl mb-10 w-full">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse min-w-max">
                    <thead>
                        <tr className="bg-slate-800/60 text-[11px] font-black uppercase tracking-widest text-slate-500">
                            <th className="px-6 py-5 w-20 text-center">Pos</th>
                            <th className="px-6 py-5">Participante</th>
                            {headerJornadas.map(h => (
                                <th key={h.id} className="px-4 py-5 text-center w-20">{h.label}</th>
                            ))}
                            <th className="px-10 py-5 text-right text-[#ffd300] w-28">Total</th>
                        </tr>
                    </thead>
                        ...
                        {rankingData.map((row, i) => (
                            <tr key={row.username} className="hover:bg-white/[0.03] transition-colors">
                                <td className="px-6 py-4 text-center font-bold text-lg">
                                    {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : <span className="text-slate-500 text-sm">{i + 1}º</span>}
                                </td>
                                <td className="px-6 py-4 font-black uppercase text-base text-white tracking-tight">
                                    {row.username}
                                </td>
                                {headerJornadas.map(h => (
                                    <td key={h.id} className="px-4 py-4 text-center text-slate-400 font-mono text-base">
                                        {row.breakdown[h.label]}
                                    </td>
                                ))}
                                <td className="px-10 py-4 text-right font-black italic text-2xl" style={{ color: i === 0 ? '#FFD300' : '#FFFFFF' }}>
                                    {row.total}
                                </td>
                            </tr>
                        ))}
