...
/* COMPONENTE DE RANKING ADMIN - USANDO USER_POINTS */
export default function RankingView() {
    ...
    useEffect(() => {
        const fetchRanking = async () => {
            ...
            // 1. Obtenemos los usuarios (excluyendo admins)
            const { data: appUsers } = await supabase
                .from('app_users')
                .select('id, username')
                .neq('role', 'admin');

            // 2. Obtenemos las jornadas (todas las configuradas para el desglose)
            ...
            // 4. Construimos el desglose mapeando los puntos de la tabla
            const userScores = appUsers.map(u => {
                let total = 0;
                const dayBreakdown: Record<number, number> = {};

                // Filtramos los puntos registrados para este usuario específico
            ...
            // 5. Ordenamos: Mayor puntaje primero, luego orden alfabético
            userScores.sort((a, b) => {
            ...
            <div className="w-full flex items-center justify-between mb-4 px-4 md:px-12">
                <div className="flex-1 flex justify-start">
                    <button 
                        ...
                        className={`px-6 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.2em] italic transition-all duration-500 border ${showFull ? 'bg-white text-black border-white' : 'bg-transparent text-white border-white/20'}`}
                    >
                        {showFull ? '← VOLVER' : 'DESGLOSE'}
                    </button>
                </div>

                <h2 className="text-2xl font-black italic uppercase tracking-tighter text-center px-4">
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center bg-black/40 rounded border border-white/10 overflow-hidden">
                            <button 
                                ...
                                className={`px-5 py-2 text-xs font-black transition-colors border-r border-white/10 ${safeCurrentPage === 0 ? 'opacity-20' : 'text-[#FFD300] hover:bg-white/10'}`}
                            >◀</button>
                            <button 
                                disabled={safeCurrentPage === totalPages - 1} 
                                onClick={() => setCurrentPage(prev => prev + 1)} 
                                className={`px-5 py-2 text-xs font-black transition-colors ${safeCurrentPage === totalPages - 1 ? 'opacity-20' : 'text-[#FFD300] hover:bg-white/10'}`}
                            >▶</button>
                        </div>
                    )}
                </div>
                    ...
                    <table className="border-collapse table-auto">
                        <tbody>
                            {paginatedUsers.map((u, idx) => {
                                ...
                                return (
                                    <tr key={u.username} className={`border-b border-white/5 hover:bg-white/[0.03] transition-colors group ${isFirst ? 'bg-[#FFD300]/5' : ''}`}>
                                        <td className="w-10 px-2 py-1 text-center border-r border-white/5 font-black italic text-[10px]">
                                            {isFirst ? <span className="text-lg">👑</span> : <span className="text-slate-600">{globalPos}</span>}
                                        </td>
                                        
                                        <td className="w-[130px] px-2 py-1 border-r border-white/5">
                                            <div className="flex items-center gap-2">
                                                <div className={`relative w-7 h-7 rounded-full overflow-hidden border shrink-0 flex items-center justify-center bg-slate-800 font-bold text-[10px] ${isFirst ? 'border-[#FFD300]' : 'border-white/10 text-slate-400'}`}>
                                                    {!hasError ? (
                                                        <img 
                                                ...
                                                <span className={`uppercase text-[10px] tracking-[0.1em] truncate block w-full ${isFirst ? 'text-[#FFD300] font-black' : 'text-slate-300'}`}>
                                                    {u.username}
                                                </span>
                                            </div>
                                        </td>
