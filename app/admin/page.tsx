...
type AdminCountry = 'spain' | 'mexico' | 'brazil'
const ADMIN_COUNTRIES: { key: AdminCountry; flag: string; name: string }[] = [
...
function AdminDashboardInner() {
    ...
    const [country, setCountry] = useState<AdminCountry>('spain')
    ...
    // Kings: Spain/Mexico/Brazil. Queens: Spain only (no other tabs shown)
    const isKingsTab = tab === 'kings'
    const isQueensTab = tab === 'queens'
        ...
        <div className="min-h-screen bg-[#0a0a0a] text-white w-full">
            ...
            {isKingsTab && (
                <div className="flex gap-2 px-4 pt-4 max-w-5xl mx-auto overflow-x-auto pb-1">
                    {ADMIN_COUNTRIES.map(({ key, flag, name }) => (
                        <button
                            key={key}
...
function CompetitionAdmin({ competitionKey }: { competitionKey: string }) {
    ...
    // 2. Cargar predicciones de jornada activa y página actual
    useEffect(() => {
                ...
                <div className="relative group w-full mb-8">
                    <div className="w-full px-4 md:px-10 py-4 grid grid-cols-1 sm:grid-cols-3 items-center gap-3 bg-slate-900/40 border-b border-white/5">
                        ...
                        <div className="flex justify-center">
                            <h3 style={{ color: competitionKey === 'kings' ? '#ffd300' : '#01d6c3' }} className="text-2xl md:text-3xl font-black italic uppercase tracking-tighter">
                                {activeMatchday.name}
                            </h3>
                        </div>
                        <div className="flex justify-center sm:justify-end gap-2 flex-wrap">
                            <button onClick={()=>toggleVisible(activeMatchday.id, activeMatchday.is_visible)} className={`px-4 md:px-6 py-2 text-xs font-black rounded-full border ${activeMatchday.is_visible ? 'bg-green-600 border-green-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>{activeMatchday.is_visible ? 'PÚBLICO' : 'OCULTO'}</button>
                            <button onClick={()=>toggleLock(activeMatchday.id, activeMatchday.is_locked)} className={`px-4 md:px-6 py-2 text-xs font-black rounded-full border ${activeMatchday.is_locked ? 'bg-red-600 border-red-400' : 'bg-blue-600 border-blue-400'}`}>{activeMatchday.is_locked ? 'BLOQUEADO' : 'ABIERTO'}</button>
                        </div>
                    </div>
...
function RankingView() {
    ...
    return (
        <div className="w-full flex flex-col items-center py-2 px-4 md:px-6">
            ...
            <div className="w-full flex flex-col sm:flex-row items-center justify-between mb-4 gap-3 px-2 md:px-12">
                <button 
