...
export default function AdminDashboard() {
    ...
    const handleLogout = () => { localStorage.removeItem('muertazos_user'); router.push('/') }
    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white w-full overflow-x-hidden">
            ...
            <header className="w-full h-16 md:h-20 flex items-center justify-between bg-slate-950 border-b border-slate-800 shadow-lg px-4 md:px-8 sticky top-0 z-50">
                ...
                {/* Izquierda: KINGS · QUEENS  (desktop) + Hamburger (mobile) */}
                ...
                <div className="flex-shrink-0 flex justify-center items-center">
                    <div className="relative w-32 h-9 md:w-44 md:h-12 hover:scale-105 transition-transform duration-300 cursor-pointer">
                        <Image src="/Muertazos.png" alt="Muertazos Logo" fill className="object-contain" priority />
                    </div>
                </div>

                {/* Derecha: RANKING · SIMULADOR · SALIR  (desktop) */}
                <div className="flex items-center gap-6 flex-1 justify-end">
                    ...
                    <button
                        ...
                        className="bg-red-600/10 text-red-500 border border-red-500/30 px-4 py-1.5 md:px-6 md:py-2 rounded-lg font-black hover:bg-red-600 hover:text-white transition-all text-[10px] uppercase italic tracking-[0.2em]"
                    >
                        SALIR
                    </button>
                </div>
                ...
                <div className="fixed inset-0 z-[60] lg:hidden">
                    ...
                    <div className="absolute left-0 top-0 bottom-0 w-64 bg-slate-950 border-r border-slate-800 flex flex-col">
                        ...
                        <nav className="flex-1 flex flex-col gap-1 p-4">
                            {[
                                { label: 'KINGS',     tab: 'kings'     as const, color: '#ffd300' },
                                { label: 'QUEENS',    tab: 'queens'    as const, color: '#01d6c3' },
                                { label: 'RANKING',   tab: 'ranking'   as const, color: '#ffffff' },
                                { label: 'SIMULADOR', tab: 'simulator' as const, color: '#FF5733' },
                            ].map(item => (
                                <button
                                    key={item.tab}
                                    onClick={() => { setTab(item.tab); setMenuOpen(false) }}
                                    style={{ color: tab === item.tab ? item.color : undefined }}
                                    className={`text-left px-4 py-3 rounded-xl font-black italic text-lg uppercase tracking-tight transition-all ${
                                        tab === item.tab ? 'bg-white/5' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.03]'
                                    }`}
                                >
                                    {item.label}
                                </button>
...
/* ──────────────────────────────────────────────────────────
   TAB BUTTON
────────────────────────────────────────────────────────── */
function TabBtn({ label, active, onClick, activeColor }: {
...
function CompetitionAdmin({ competitionKey }: { competitionKey: string }) {
    ...
    // 2. PREDICCIONES DE LA JORNADA ACTIVA
    useEffect(() => {
    ...
    const toggleVisible = async (id: number, currentVal: boolean) => {
        ...
        await supabase.from('matchdays').update({ is_visible: !currentVal }).eq('id', id)
        load()
    }
        ...
        <div className="w-full flex flex-col items-center overflow-x-hidden">
            ...
            {activeMatchday && (
                <div className="relative group w-full mb-8">
                    {/* Barra de control de jornada */}
                    <div className="w-full px-4 md:px-10 py-3 grid grid-cols-3 items-center bg-slate-900/40 border-b border-white/5">
                        <div className="flex justify-start">
                            {totalPages > 1 && (
                                <div className="flex items-center bg-black/40 rounded border border-white/10 overflow-hidden">
                                    <button disabled={currentPage === 0} onClick={() => setCurrentPage(p => p - 1)}
                                        className={`px-4 py-2 text-xs font-black transition-colors border-r border-white/10 ${currentPage === 0 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>◀</button>
                                    <button disabled={currentPage === totalPages - 1} onClick={() => setCurrentPage(p => p + 1)}
                                        className={`px-4 py-2 text-xs font-black transition-colors ${currentPage === totalPages - 1 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>▶</button>
                                </div>
                            )}
                        </div>
                        <div className="flex justify-center">
                            <h3
                                ...
                                className="text-xl md:text-3xl font-black italic uppercase tracking-tighter"
                            >
                                {activeMatchday.name}
                            </h3>
                        </div>
                        <div className="flex justify-end gap-2 md:gap-4">
                            <button
                                ...
                                className={`px-3 md:px-6 py-1.5 text-[10px] font-black rounded-full border ${activeMatchday.is_visible ? 'bg-green-600 border-green-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}
                            >
                                {activeMatchday.is_visible ? 'PÚBLICO' : 'OCULTO'}
                            </button>
                            <button
                                onClick={() => toggleLock(activeMatchday.id, activeMatchday.is_locked)}
                                className={`px-3 md:px-6 py-1.5 text-[10px] font-black rounded-full border ${activeMatchday.is_locked ? 'bg-red-600 border-red-400' : 'bg-blue-600 border-blue-400'}`}
                            >
                    ...
                    <div className="w-full overflow-x-auto">
                        <table className="w-full border-collapse table-fixed text-center" style={{ minWidth: 600 }}>
                            <thead>
...
function RankingView() {
    ...
    useEffect(() => {
        const fetchRanking = async () => {
            const { data: lockedDays } = await supabase
                .from('matchdays').select('id, name, competition_key').eq('is_locked', true).order('display_order')
            ...
            const { data: appUsers } = await supabase.from('app_users')
                .select('id, username').neq('role', 'admin')
    ...
    return (
        <div className="w-full flex flex-col items-center py-4 px-4 overflow-x-hidden">
            {/* Controles */}
            <div className="w-full flex items-center justify-between mb-4 px-2 md:px-10">
                <div className="flex-1 flex justify-start">
                    <button
                        onClick={() => setShowFull(!showFull)}
                        className={`px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-[0.2em] italic transition-all border ${showFull ? 'bg-white text-black border-white' : 'bg-transparent text-white border-white/20 hover:border-[#FFD300] hover:text-[#FFD300]'}`}
                    >
                    ...
                    {totalPages > 1 && (
                        <div className="flex items-center bg-black/40 rounded border border-white/10 overflow-hidden">
                            <button disabled={safePage === 0} onClick={() => setCurrentPage(p => p - 1)}
                                className={`px-4 py-2 text-xs font-black transition-colors border-r border-white/10 ${safePage === 0 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>◀</button>
                            <button disabled={safePage === totalPages - 1} onClick={() => setCurrentPage(p => p + 1)}
                                className={`px-4 py-2 text-xs font-black transition-colors ${safePage === totalPages - 1 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>▶</button>
                        </div>
                    )}
                </div>
            </div>
                            ...
                            {paginated.map((user, idx) => {
                                ...
                                return (
                                    <tr key={user.username} className={`border-b border-white/5 hover:bg-white/[0.03] transition-colors group ${isFirst ? 'bg-[#FFD300]/5' : ''}`}>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className={`relative w-7 h-7 rounded-full overflow-hidden border shrink-0 shadow-md flex items-center justify-center bg-slate-800 font-bold text-[10px] ${isFirst ? 'border-[#FFD300]' : 'border-white/10 text-slate-400'}`}>
                                                    ...
                                                    <Image
                                                        ...
                                                        alt={user.username} fill sizes="28px"
                                        ...
                                        <td className={`w-16 px-2 py-1.5 text-center border-l border-white/10 font-black text-base italic ${isFirst ? 'bg-[#FFD300] text-black' : 'bg-[#FFD300]/5 text-[#FFD300]'}`}>
                                            {user.total}
                                        </td>
                                    </tr>
                                )
...
function SimulatorView() {
    ...
    const saveActiveMatchday = async () => {
        ...
        const pending = resultsToUpsert.some((r: any) => r.home_goals === r.away_goals && r.home_penalties === 0 && r.away_penalties === 0)
        if (pending) return alert('Selecciona al ganador de los penales haciendo clic en su escudo.')
    ...
    return (
        <div className="w-full flex flex-col items-center overflow-x-hidden">
            ...
            {/* Selector jornada */}
            <div className="w-full flex justify-center flex-wrap gap-2 py-2 px-4 border-b border-white/5 bg-slate-900/20">
                {matchdays.map(day => (
                    <button key={day.id} onClick={() => setActiveMatchdayId(day.id)}
                        className={`px-3 py-1 text-[11px] font-black italic uppercase rounded border ${activeMatchdayId === day.id ? (compKey === 'kings' ? 'bg-[#FFD300] text-black' : 'bg-[#01d6c3] text-black') : 'bg-black/40 text-slate-400'}`}>
                        {day.name}
                    </button>
                ))}
            </div>
