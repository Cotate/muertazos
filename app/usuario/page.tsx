...
export default function UserDashboard() {
  ...
  const loadData = async () => {
    ...
    if (mDays) {
      ...
      const currentIds = sorted[currentDayIndex]?.matches.map((m: any) => m.id) || []
      setHasSavedInDB(!!(preds?.some((p: any) => currentIds.includes(p.match_id))))
    }
  }
  ...
  const NavButton = ({ label, targetView, targetLeague }: { label: string; targetView: any; targetLeague?: any }) => {
    ...
    const color = isActive
      ? (targetLeague === 'queens' || (!targetLeague && league === 'queens') ? '#01d6c3' : '#ffd300')
      : undefined
    return (
      <button
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans overflow-x-hidden">
      ...
      <header className="w-full h-16 md:h-20 flex items-center justify-between bg-slate-950 border-b border-slate-800 shadow-lg px-4 md:px-8 sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4 flex-1">
          <button onClick={() => setMenuOpen(!menuOpen)} className="lg:hidden text-white p-1.5 rounded-lg hover:bg-white/10 transition-colors" aria-label="Menú">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-64 bg-slate-950 border-r border-slate-800 flex flex-col">
            ...
            <nav className="flex-1 flex flex-col gap-1 p-4">
              {[
                ...
                <button
                  key={`${item.view}-${item.league ?? ''}`}
                  onClick={() => {
                    setView(item.view)
                    if (item.league) setLeague(item.league)
                    setMenuOpen(false)
                  }}
                  style={{ color: view === item.view && (!item.league || league === item.league) ? item.color : undefined }}
                  className={`text-left px-4 py-3 rounded-xl font-black italic text-lg uppercase tracking-tight transition-all ${
                    view === item.view && (!item.league || league === item.league) ? 'bg-white/5' : 'text-slate-500 hover:text-slate-300 hover:bg-white/[0.03]'
                  }`}
                >
                  {item.label}
                </button>
      ...
      {/* ══════════════════ CONTENIDO ══════════════════ */}
      <main className={`mx-auto pt-6 pb-20 overflow-x-hidden ${view === 'all-picks' ? 'w-full' : 'max-w-5xl px-4'}`}>

        {/* ── PICKS ── */}
        {view === 'picks' ? (
          <div className="max-w-2xl mx-auto bg-slate-900/40 rounded-3xl p-5 md:p-6 border border-slate-800 shadow-2xl backdrop-blur-sm">
            {matchdays.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64">
                <p className="text-slate-600 font-black italic tracking-widest animate-pulse">PRÓXIMAMENTE...</p>
              </div>
            ) : (
              <>
                {/* Navegación entre jornadas */}
                <div className="flex justify-between items-center mb-6 bg-slate-950/50 p-3 md:p-4 rounded-2xl border border-slate-800">
                  <button
                    disabled={currentDayIndex === 0}
                    onClick={() => { setCurrentDayIndex(i => i - 1); setIsEditing(false) }}
                    style={{ color: activeColor, borderColor: activeColor + '40' }}
                    className="w-10 h-10 flex items-center justify-center border rounded-xl disabled:opacity-10 font-black text-xl hover:bg-white/5 transition-colors"
                  >←</button>
                  <div className="text-center">
                    <h2 className="text-xl md:text-2xl font-black italic uppercase tracking-tighter" style={{ color: activeColor }}>
                ...
                <div className="space-y-3">
                  {matchdays[currentDayIndex].matches.map((match: any) => {
                    ...
                    return (
                      <div key={match.id} className="flex justify-between items-center bg-slate-950/40 p-4 md:p-5 rounded-2xl border border-slate-800/50 hover:border-slate-700 transition-colors">
                        ...
                        <span className="text-2xl md:text-3xl font-black text-white italic tracking-tighter mx-2">VS</span>
...
/* ──────────────────────────────────────────────────────────
   COMPETITION READ-ONLY (TABLA DE PICKS)
────────────────────────────────────────────────────────── */
function CompetitionReadOnly({ competitionKey }: { competitionKey: string }) {
  ...
  if (matchdays.length === 0) return (
    <div className="w-full text-center py-40 border-2 border-dashed border-white/5 rounded-3xl">
      <p className="text-slate-600 font-black italic text-xl md:text-2xl uppercase tracking-widest">No hay jornadas cerradas</p>
    </div>
  )
    ...
    <div className="w-full flex flex-col items-center overflow-x-hidden">
      ...
      {activeMatchday && (
        <div className="w-full">
          {/* Encabezado jornada + paginación */}
          <div className="w-full px-4 py-4 flex flex-row justify-center items-center gap-3 md:gap-8 bg-gradient-to-b from-black/20 to-transparent">
            {totalPages > 1 && (
              <div className="flex items-center bg-black/60 rounded-xl border border-white/10 overflow-hidden shadow-2xl">
                <button disabled={currentPage === 0} onClick={() => setCurrentPage(p => p - 1)}
                  className={`px-5 py-2 text-sm font-black transition-colors border-r border-white/10 ${currentPage === 0 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>◀</button>
                <span className="px-3 text-[10px] text-slate-500 font-bold uppercase tracking-tighter">
                  {currentPage + 1}/{totalPages}
                </span>
                <button disabled={currentPage === totalPages - 1} onClick={() => setCurrentPage(p => p + 1)}
                  className={`px-5 py-2 text-sm font-black transition-colors ${currentPage === totalPages - 1 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>▶</button>
              </div>
            )}
            <h3
              style={{ color: competitionKey === 'kings' ? '#ffd300' : '#01d6c3' }}
              className="text-2xl md:text-4xl font-black italic uppercase tracking-tighter drop-shadow-2xl"
            >
              {activeMatchday.name}
            </h3>
          </div>
          ...
          <div className="w-full overflow-x-auto pb-10">
            <table className="w-full border-collapse table-fixed text-center" style={{ minWidth: 700 }}>
              <thead>
                ...
                {activeMatchday.matches?.map((m: any) => (
                  <tr key={m.id} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                    ...
                    {paginatedUsers.map(u => {
                      ...
                      return (
                        <td key={u.id} className="p-1.5 border-r border-white/5">
...
function RankingView({ user }: { user: any }) {
  ...
  for (let i = 0; i < totalUsers; i += 15) pageChunks.push([i, Math.min(i + 15, totalUsers)])
          ...
          {totalPages > 1 && (
            <div className="flex items-center bg-black/40 rounded border border-white/10 overflow-hidden">
              <button disabled={safePage === 0} onClick={() => setCurrentPage(p => p - 1)}
                className={`px-5 py-2 text-xs font-black transition-colors border-r border-white/10 ${safePage === 0 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>◀</button>
              <button disabled={safePage === totalPages - 1} onClick={() => setCurrentPage(p => p + 1)}
                className={`px-5 py-2 text-xs font-black transition-colors ${safePage === totalPages - 1 ? 'opacity-20' : 'hover:bg-white/10 text-[#FFD300]'}`}>▶</button>
            </div>
          )}
        </div>
      </div>
              ...
              {paginated.map((u, idx) => {
                ...
                return (
                  <tr key={u.username} className={`border-b border-white/5 hover:bg-white/[0.03] transition-colors group ${isFirst ? 'bg-[#FFD300]/5' : ''} ${isMe ? 'bg-blue-500/10' : ''}`}>
                    ...
                    <td className="w-[130px] px-3 py-1.5 border-r border-white/5">
                    ...
                    <td className={`w-16 px-4 py-1.5 text-center border-l border-white/10 font-black text-sm italic ${isFirst ? 'bg-[#FFD300] text-black' : isMe ? 'bg-white/10 text-white' : 'bg-[#FFD300]/5 text-[#FFD300]'}`}>
                      {u.total}
                    </td>
                  </tr>
                )
  ...
  return (
    <div className="w-full flex flex-col items-center overflow-x-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 md:px-6 py-8">
        <div className="mb-6">
          <h3 className="text-xl md:text-2xl font-black italic uppercase tracking-tighter text-center xl:text-left">{activeMatchday?.name}</h3>
        </div>
                ...
                <div key={m.id} className="bg-slate-900/50 border border-white/10 rounded-xl p-4 flex flex-col items-center gap-4">
                  <div className="w-full flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                      <input type="text" value={s.hg} onChange={(e) => handleLocalScoreChange(m.id, 'hg', e.target.value)} className="w-10 h-10 md:w-12 md:h-12 text-center bg-black border border-white/20 rounded-md font-black text-xl text-white focus:border-[#FFD300] focus:outline-none" maxLength={2} />
                      <span className="text-xs font-black text-white italic">VS</span>
                      <input type="text" value={s.ag} onChange={(e) => handleLocalScoreChange(m.id, 'ag', e.target.value)} className="w-10 h-10 md:w-12 md:h-12 text-center bg-black border border-white/20 rounded-md font-black text-xl text-white focus:border-[#FFD300] focus:outline-none" maxLength={2} />
                    </div>
                    <div className="flex flex-col items-center flex-1">
                      {m.away && (
...
function PizarraView() {
  ...
  const addAllPlayers = () => {
    ...
    setPlayersOnPitch(prev => [...prev, ...players.map((fileName, i) => ({ id: Math.random().toString(36).substr(2, 9), team: selectedTeam, fileName, x: 50, y: 50, zIndex: count + i + 1 }))])
  }
  const handlePointerDown = (id: string) => {
    setDraggingId(id)
    setPlayersOnPitch(prev => { const maxZ = prev.length > 0 ? Math.max(...prev.map(p => p.zIndex)) : 0; return prev.map(p => p.id === id ? { ...p, zIndex: maxZ + 1 } : p) })
  }
